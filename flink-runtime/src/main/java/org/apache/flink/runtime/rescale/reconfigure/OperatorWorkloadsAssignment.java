...
public class OperatorWorkloadsAssignment implements AbstractCoordinator.Diff {
    ...
    public OperatorWorkloadsAssignment(
        ...
        checkState(
                checkPartitionAssignmentValidity(executorMapping),
                "executorMapping has null or empty partition");

        checkState(
                checkPartitionAssignmentValidity(oldExecutorMapping),
    ...
    public OperatorWorkloadsAssignment(
            Map<Integer, List<Integer>> executorMapping, int numOpenedSubtask) {
    ...
    private void setupFollowScaleOut(
        ...
        List<Integer> createdExecutorIdList =
        ...
        List<Integer> modifiedExecutorIdList =
                oldExecutorMapping.keySet().stream()
                        //			.filter(id -> oldExecutorMapping.get(id).size() !=
                        // executorMapping.get(id).size())
                        .filter(
                                id ->
                                        // listEqualsIgnoreOrder(executorMapping.get(id),
                                        // oldExecutorMapping.get(id))
                                        !(executorMapping.get(id).size()
                                                        == oldExecutorMapping.get(id).size()
                                                && executorMapping
                                                        .get(id)
                                                        .containsAll(oldExecutorMapping.get(id))))
                        .collect(Collectors.toList());
        ...
        for (Map.Entry<Integer, List<Integer>> entry : executorMapping.entrySet()) {
            ...
            // if the subtask is to be created, find out the corresponding subtask index from
            // unUsedSubtaskMap
            int subtaskIndex =
                    (createdExecutorIdList.contains(executorId))
            ...
            if (createdExecutorIdList.contains(executorId)
                    || modifiedExecutorIdList.contains(executorId)) {
    ...
    private void setupFollowScaleIn(
        ...
        List<Integer> removedExecutorIdList =
                oldExecutorMapping.keySet().stream()
        ...
        List<Integer> modifiedExecutorIdList =
                executorMapping.keySet().stream()
                        //			.filter(id -> executorMapping.get(id).size() !=
                        // oldExecutorMapping.get(id).size())
                        .filter(
                                id ->
                                        // listEqualsIgnoreOrder(executorMapping.get(id),
        ...
        for (Map.Entry<Integer, List<Integer>> entry : executorMapping.entrySet()) {
            ...
            if (modifiedExecutorIdList.contains(executorId)
                    || removedExecutorIdList.contains(executorId)) {
                modifiedSubtaskMap.put(subtaskIndex, true);
            }
        }
    }
    ...
    private void setupFollowRepartition(
        ...
        // the state can be shuffled among all tasks, so the size of keys in each task can be the
        // same.
        List<Integer> modifiedExecutorIdList =
                executorMapping.keySet().stream()
                        .filter(
                                id -> {
                                    ...
                                    //				listEqualsIgnoreOrder(executorMapping.get(id),
                                    // oldExecutorMapping.get(id));
                                    return !(executorMapping.get(id).size()
                                                    == oldExecutorMapping.get(id).size()
                                            && executorMapping
                                                    .get(id)
                                                    .containsAll(oldExecutorMapping.get(id)));
                                    //				return
                                    // CollectionUtils.isEqualCollection(executorMapping.get(id),
                                    // executorMapping.get(id));
                                })
                        .collect(Collectors.toList());
        //		checkState(modifiedExecutorIdList.size() == 2, "not exactly two are modified in
        // repartition");
    ...
    private void setupFollowPlacement(
            Map<Integer, List<Integer>> executorMapping,
            Map<Integer, List<Integer>> oldExecutorMapping) {
        List<Integer> removedExecutorIdList =
                oldExecutorMapping.keySet().stream()
        ...
        Map<Integer, Integer> unUsedSubtaskMap =
                findNextUnusedSubtask(createdExecutorIdList, removedExecutorIdList);
    ...
    private Map<Integer, Integer> findNextUnusedSubtask(List<Integer> createdExecutorIdList) {
        ...
        for (int i = 0; i < numOpenedSubtask; i++) {
            if (oldRescalePA.getIdInModel(i) == UNUSED_SUBTASK) {
                unUsedSubtaskMap.put(
                        createdExecutorIdList.get(createdExecutorIdList.size() - n), i);
                n--;
                if (n == 0) {
                    break;
                }
    ...
    private Map<Integer, Integer> findNextUnusedSubtask(
            List<Integer> createdExecutorIdList, List<Integer> removedExecutorIdList) {
        checkState(createdExecutorIdList.size() > 0, "null created task list");
    ...
    private void fillingUnused(int newParallelism) {
        ...
        for (int subtaskIndex = 0; subtaskIndex < numOpenedSubtask; subtaskIndex++) {
            ...
            partitionAssignment.putIfAbsent(subtaskIndex, new ArrayList<>());
            if (absent != null) {
                newSubtaskIndexMapping.put(
                        numOccupiedSubtask, subtaskIndexMapping.get(subtaskIndex));
                newPartitionAssignment.put(
                        numOccupiedSubtask, partitionAssignment.get(subtaskIndex));
    ...
    private void generateAlignedKeyGroupRanges() {
        ...
        for (int subTaskIndex = 0;
                subTaskIndex < partitionAssignment.keySet().size();
                subTaskIndex++) {
    ...
    private static Map<Integer, Integer> initSubtaskIndexMap(int numExecutors) {
