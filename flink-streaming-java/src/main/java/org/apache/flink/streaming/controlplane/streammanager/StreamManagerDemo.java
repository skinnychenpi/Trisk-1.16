...
// import
// org.apache.flink.streaming.controlplane.jobgraph.DefaultExecutionPlanAndJobGraphUpdaterFactory;
...
//
// import static
// org.apache.flink.runtime.controlplane.abstraction.OperatorDescriptor.ExecutionLogic.UDF;
...
public class StreamManagerDemo {}

// public class StreamManager extends FencedRpcEndpoint<StreamManagerId> implements
// StreamManagerGateway, StreamManagerService, ReconfigurationExecutor {
//
//
...
// streamManagerConfiguration.getConfiguration().getString(CONTROLLER, "DummyController");
//        switch (controllerName) {
//            case "DummyController":
//                this.controlPolicyList.put("DummyController", new DummyController(this));
//                break;
//            case "StockController":
//                this.controlPolicyList.put("StockController", new StockController(this,
// streamManagerConfiguration.getConfiguration()));
//                break;
//            case "NexmarkController":
//                this.controlPolicyList.put("NexmarkController", new NexmarkController(this,
// streamManagerConfiguration.getConfiguration()));
//                break;
//            case "PerformanceEvaluator":
//                this.controlPolicyList.put("PerformanceEvaluator", new PerformanceEvaluator(this,
// streamManagerConfiguration.getConfiguration()));
//                break;
...
//        }
//
//        reconfigurationProfiler = new
// ReconfigurationProfiler(streamManagerConfiguration.getConfiguration());
//    }
//
//    /**
...
//     */
//    @Override
//    public CompletableFuture<Acknowledge> start(StreamManagerId newStreamManagerId) throws
// Exception {
...
//        start();
//
//        return callAsyncWithoutFencing(() -> startStreamManagement(newStreamManagerId),
...
//            } catch (Exception e) {
//                StreamManagerException exception = new StreamManagerException("Could not add the
// job " +
...
//            onFatalError(exception);
//
//            log.debug("Could not obtain the job leader id future to verify the correct job
// leader.");
//            return FutureUtils.completedExceptionally(exception);
//        }
//
...
//                                "The leading JobMaster id %s did not match the received JobMaster
// id %s. " +
//                                        "This indicates that a JobMaster leader change has
// happened.",
//                                leadingJobMasterId,
//                                jobMasterId);
...
//                            log.debug("Registration of job manager {}@{} failed.", jobMasterId,
// jobManagerAddress, throwable);
//                        } else {
//                            log.info("Registration of job manager {}@{} failed.", jobMasterId,
// jobManagerAddress);
//                        }
//
...
//
////		checkState(this.rescalePartitionFuture.isDone(), "Last rescale/repartition action haven't
// done");
...
//        runAsync(() -> jobMasterGateway.triggerJobRescale(wrapper, jobGraph, upDownStream.f0,
// upDownStream.f1));
//    }
//
//    public void rescale(int operatorID, int newParallelism, Map<Integer, List<Integer>>
// keyStateAllocation, ControlPolicy waitingController) {
//        try {
...
//            // although we put the update key mapping in the target operator, it will find the
// upstream tasks and update.
...
//                                    return coordinator.synchronizeTasks(tasks, o); // CYH: This is
// the synchronizeTask method in ReconfigurationCoordinator.
//                                });
...
//
//    @Override
//    public void rescale(ExecutionPlan executionPlan, int operatorID, Boolean isScaleIn,
...
//
//    @Override
//    public void placement(int operatorID, Map<Integer, List<Integer>> keyStateAllocation,
// ControlPolicy waitingController) {
//        try {
//            reconfigurationProfiler.onReconfigurationStart();
...
//
//    @Override
//    public void rebalance(int operatorID, Map<Integer, List<Integer>> keyStateAllocation, boolean
// stateful, ControlPolicy waitingController) {
//        try {
//            reconfigurationProfiler.onReconfigurationStart();
//            // typically, the target operator should contain key state,
//            // todo if keyStateAllocation is null, means it is stateless operator, but not support
// now
//            this.trisk.setStateUpdatingFlag(waitingController);
//
//            OperatorDescriptor targetDescriptor = trisk.getOperatorByID(operatorID);
...
//                                        reconfigurationProfiler.onOtherEnd(SYN);
//                                        reconfigurationProfiler.onOtherStart(UPDATE_MAPPING);
//                                        return coordinator.updateKeyMapping(updateKeyMappingTasks,
// o);
//                                    })
////						.thenCompose(o -> {
////						reconfigurationProfiler.onOtherEnd(UPDATE_MAPPING);
...
//                            // finish the reconfiguration after all asynchronous update completed
//                            return FutureUtils.completeAll(updateFutureList)
////						.thenCompose(o -> coordinator.resumeTasks()) // resume all tasks who has both state and
// mapping to update (to be appeared in the future).
//                                    .whenComplete((o, failure) -> {
//                                        if (failure != null) {
//                                            failure.printStackTrace();
//                                        }
...
//
//    @Override
//    public void rebalance(ExecutionPlan executionPlan, int operatorID, ControlPolicy
// waitingController) {
//        try {
//            reconfigurationProfiler.onReconfigurationStart();
//            // typically, the target operator should contain key state,
...
//
//    @Override
//    public void reconfigureUserFunction(int operatorID, Object function, ControlPolicy
// waitingController) {
//        try {
...
//    }
//
//    public void callCustomizeOperations(Function<PrimitiveOperation<Map<Integer, Map<Integer,
// AbstractCoordinator.Diff>>>, CompletableFuture<?>> operationCaller) {
//        try {
//            JobMasterGateway jobMasterGateway =
// this.jobManagerRegistration.getJobManagerGateway();
...
//            byte[] byteCodeArray = ByteClassLoader.compileJavaClass(funcClassName, sourceCode,
// options);
...
//            log.info("register new function class: " + funcClassName + " store with blob key: " +
// blobKey);
...
//
//    @Override
//    public void jobStatusChanged(JobID jobId, JobStatus newJobStatus, long timestamp, Throwable
// error, ExecutionPlan jobAbstraction) {
//        runAsync(
//                () -> {
...
//    }
//
//    public boolean registerNewController(String controllerID, String className, String
// sourceCode){
...
//            log.info("new registered controller started: " + controllerID + ", type: " +
// newController);
...
//            if (Objects.equals(oldJobManagerRegistration.getJobMasterId(),
// jobMasterGateway.getFencingToken())) {
//                // same registration
//                log.debug("Job manager {}@{} was already registered.",
// jobMasterGateway.getFencingToken(), jobManagerAddress);
//            } else {
...
//        }
//
//        log.info("Registered job manager {}@{} for job {}.", jobMasterGateway.getFencingToken(),
// jobManagerAddress, jobId);
//
...
//                        // todo, how to know job success run so we can start something related
// control plane (eg. streamSwitch)
...
//        return checkNotNull(trisk.getExecutionPlan(), "stream job abstraction (execution plan)
...
//                        // run update asynchronously.
//                        List<CompletableFuture<?>> updateFutureList = new ArrayList<>();
//                        // TODO: by far, our method only support one operator, need to extend to
...
//            } else {
//                log.debug("Discarding job leader lost leadership, because a new job leader was
// found for job {}. ", jobId);
//            }
//        } else {
//            log.debug("Discard job leader lost leadership for outdated leader {} for job {}.",
// oldJobMasterId, jobId);
//        }
//    }
//
...
//        } catch (Exception e) {
//            log.warn("Could not properly remove the job {} from the job leader id service.",
// jobId, e);
//        }
//
//        if (jobId == this.jobId) {
