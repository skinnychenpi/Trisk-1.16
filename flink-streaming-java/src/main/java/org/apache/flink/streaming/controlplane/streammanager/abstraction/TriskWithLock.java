 ...
 * of the whole dataflow, 2. key mappings: mappings of upstream key groups to downstream tasks, 3.
 * key state allocation: key groups allocation among tasks in the same stage, 4. user defined
 * function:user defined execution logic of each task, 5. nThreads: num-ber of threads owned by each
 * task, 6. task location: location of one task in cluster.
 *
 * <p>G(V,E) is a graph with a set of vertices V connected by a set of edges E. G(V,E) describes the
 * operator level abstraction. V contains the execution logic configurations: user defined function
 * and key state allocation. E provides the connectivity information of different vertices, the main
 * information is keymappings.
 *
 * <p>D(H,T) is the deployment of tasks of the streaming job on the cluster, it describes task level
 * abstraction. H represents the hosts in the cluster, each host has a certain number of CPU and
 * memory resources. T is the set of tasks, the main information in T is: number of threads owned by
