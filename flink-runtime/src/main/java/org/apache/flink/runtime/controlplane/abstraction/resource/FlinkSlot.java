...
public class FlinkSlot implements AbstractSlot {
    ...
    public static FlinkSlot fromTaskManagerSlot(TaskManagerSlotInformation slot) {
        String taskManagerId = slot.getSlotId().getResourceID().getResourceIdString();
        return new FlinkSlot(
                slot.getSlotId(),
                fromSlotState(slot.getState()),
                fromResourceProfile(slot.getResourceProfile()),
                taskManagerId);
    }

    public static State fromSlotState(SlotState state) {
        if (state == SlotState.FREE) {
            return State.FREE;
        } else if (state == SlotState.ALLOCATED) {
    ...
    public static SlotID toSlotId(String slotId) {
        ...
        try {
            ...
            return SlotID.getDynamicSlotID(new ResourceID(parts[0]));
        }
    }
