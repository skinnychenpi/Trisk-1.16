...
public class JobRescaleCoordinator implements JobRescaleAction, RescalepointAcknowledgeListener {
    ...
    public JobRescaleCoordinator(
        ...
        this.abstractCoordinator =
                new ReconfigurationCoordinator(jobGraph, executionGraph, scheduler);
    }
    ...
    public void setStreamManagerGateway(StreamManagerGateway streamManagerGateway) {
        checkNotNull(
                ...
                "The streamManagerGateway set to RescaleCoordinator is null.");
    ...
    public void repartition(
        ...
        LOG.info(
                "++++++ repartition job with RescaleID: "
                        + rescaleId
                        + ", taskID"
                        + vertexID
                        + ", partitionAssignment: "
                        + jobRescalePartitionAssignment);
    ...
    private void repartitionVertex(
        ...
        //
        //        // state check
        //        checkState(targetVertex.getParallelism() ==
        // jobRescalePartitionAssignment.getNumOpenedSubtask(),
        //                String.format("parallelism in targetVertex %d is not equal to number of
        // executors %d",
        //                        targetVertex.getParallelism(),
        // jobRescalePartitionAssignment.getNumOpenedSubtask()));
        //
        //        // rescale upstream and downstream
        //        final Collection<CompletableFuture<Void>> rescaleCandidatesFutures = new
        // ArrayList<>();
        //
        ...
        //            for (ExecutionVertex vertex : tasks.get(jobId).getTaskVertices()) {
        //                Execution execution = vertex.getCurrentExecutionAttempt();
        //                rescaleCandidatesFutures.add(execution.scheduleRescale(rescaleId,
        // RescaleOptions.RESCALE_PARTITIONS_ONLY, null));
        //            }
        //        }
        //
        //        for (int subtaskIndex = 0; subtaskIndex < targetVertex.getTaskVertices().length;
        // subtaskIndex++) {
        ...
        //                        execution.scheduleRescale(rescaleId, RescaleOptions.RESCALE_BOTH,
        // null));
        //            }
        //        }
        //
        ...
        //                rescaleCandidatesFutures.add(execution.scheduleRescale(rescaleId,
        // RescaleOptions.RESCALE_GATES_ONLY, null));
        //            }
        //        }
        //
        //        FutureUtils
        ...
        //                        LOG.info("++++++ Make rescalepoint with checkpointId=" +
        // checkpointId);
        //                    } catch (Exception e) {
        //                        failExecution(e);
    ...
    private void scaleOutVertex(
        ...
        //        }
        //
        //        final Map<RescaleOptions, List<ExecutionVertex>> rescaleCandidates = new
        // HashMap<>();
        //
        ...
        //        this.createCandidates = this.targetVertex.scaleOut(executionGraph.getRpcTimeout(),
        ...
        //        Collection<CompletableFuture<Execution>> allocateSlotFutures = new
        // ArrayList<>(this.createCandidates.size());
        //
        ...
        //                    try {
        //                        Collection<CompletableFuture<Void>> rescaleCandidatesFutures = new
        // ArrayList<>();
        //
        //                        // before schedule rescale, need to update produced partitions
        //                        for (Map.Entry<RescaleOptions, List<ExecutionVertex>> entry :
        // rescaleCandidates.entrySet()) {
        ...
        // targetVertex.getTaskVertices().length; subtaskIndex++) {
        //                            ExecutionVertex vertex =
        // targetVertex.getTaskVertices()[subtaskIndex];
        //                            Execution execution = vertex.getCurrentExecutionAttempt();
        //                            execution.updateProducedPartitions(rescaleId);
        //                        }
        //
        ...
        // targetVertex.getTaskVertices().length; subtaskIndex++) {
        //                            if
        // (!jobRescalePartitionAssignment.isSubtaskModified(subtaskIndex)) {
        //                                ExecutionVertex vertex =
        // targetVertex.getTaskVertices()[subtaskIndex];
        //                                Execution execution = vertex.getCurrentExecutionAttempt();
        //
        //
        // rescaleCandidatesFutures.add(execution.scheduleRescale(rescaleId,
        // RescaleOptions.RESCALE_BOTH, null));
        //                            }
        //                        }
        //
        //                        return FutureUtils
    ...
    private void handleCollectedStates(Map<OperatorID, OperatorState> operatorStates)
            throws Exception {
        switch (actionType) {
            case REPARTITION:
    ...
    private void assignNewState(Map<OperatorID, OperatorState> operatorStates)
            throws ExecutionGraphException {
        ...
        //                new StateAssignmentOperation(checkpointId, newTasks, operatorStates,
        // true);
        ...
        //
        //        Collection<CompletableFuture<Void>> rescaledFuture = new
        // ArrayList<>(targetVertex.getTaskVertices().length);
        //
        ...
        //                    checkNotNull(streamManagerGateway, "The streamManagerGateway wasn't
        // set yet, notify StreamManager failed.");
        //
        //                    LOG.info("++++++ StreamSwitch in Stream Manager notify migration
        // completed");
        //
    ...
    private void deployCreatedExecution(Map<OperatorID, OperatorState> operatorStates)
        ...
        //        Collection<CompletableFuture<Void>> rescaledFuture = new
        // ArrayList<>(targetVertex.getTaskVertices().length - allocatedExecutions.size());
        //
        //        for (int i = 0; i < targetVertex.getTaskVertices().length; i++) {
        //            ExecutionVertex vertex = targetVertex.getTaskVertices()[i];
        ...
        //                    scheduledRescale =
        // vertex.getCurrentExecutionAttempt().deploy(keyGroupRange, idInModel);
        //                } else {
        //                    Execution executionAttempt = vertex.getCurrentExecutionAttempt();
        //
        //                    scheduledRescale = executionAttempt.scheduleRescale(rescaleId,
        ...
        //                .combineAll(rescaledFuture)
        //                .thenRunAsync(() -> {
        //                    LOG.info("++++++ Deploy vertex and rescale existing vertices
        // completed");
        //                    CheckpointCoordinator checkpointCoordinator =
        // executionGraph.getCheckpointCoordinator();
        //
        //                    checkNotNull(checkpointCoordinator);
        //                    checkpointCoordinator.addVertices(createCandidates.toArray(new
        ...
        //
        //                    // notify streamSwitch that change is finished
        //                    checkNotNull(streamManagerGateway, "The jobMangerGateway wasn't set
        // yet, notify StreamManager failed.");
        //                    LOG.info("++++++ StreamSwitch in Stream Manager notify migration
        // completed");
        //
        // streamManagerGateway.streamSwitchCompleted(targetVertex.getJobVertexId());
    ...
    private void cancelOldExecution(Map<OperatorID, OperatorState> operatorStates)
            throws ExecutionGraphException {
        //        Map<JobVertexID, ExecutionJobVertex> tasks = new HashMap<>();
        //        tasks.put(targetVertex.getJobVertexId(), targetVertex);
        //
        ...
        ////				System.out.println("none keygroup assigned for current jobvertex: " +
        // vertex.toString());
        ////				scheduledRescale = executionAttempt.scheduleRescale(rescaleId,
        ...
        //
        //                    checkNotNull(checkpointCoordinator);
        //                    checkpointCoordinator.dropVertices(removedCandidates.toArray(new
        // ExecutionVertex[0]), false);
        //
        //                    if (checkpointCoordinator.isPeriodicCheckpointingConfigured()) {
        //                        checkpointCoordinator.startCheckpointScheduler();
                        ...
                        try {
                            synchronized (lock) {
                                if (inProcess) {
                                    ...
                                    if (notYetAcknowledgedTasks.isEmpty()) {
                                        // receive all required snapshot, force streamSwitch to
                                        // update metrices
                                        //
