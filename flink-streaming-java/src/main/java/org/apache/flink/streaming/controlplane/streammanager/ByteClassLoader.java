...
public class ByteClassLoader extends ClassLoader {
    ...
    protected Class<?> findClass(String className) throws ClassNotFoundException {
        if (byteDataMap.isEmpty()) throw new ClassNotFoundException("byte data is empty");
    ...
    public static byte[] compileJavaClass(
            String className, String sourceCode, @Nullable Iterable<String> options)
            throws IOException {
        ...
        Iterable<? extends JavaFileObject> compilationUnits =
                Collections.singletonList(targetSource);

        JavaCompiler compiler = ToolProvider.getSystemJavaCompiler();
        ByteArrayFileManager fileManager =
                new ByteArrayFileManager(
                        compiler.getStandardFileManager(null, null, null), targetClass);
        StringWriter sw = new StringWriter();
        JavaCompiler.CompilationTask task =
                compiler.getTask(sw, fileManager, null, options, null, compilationUnits);
        if (!task.call()) {
            fileManager.close();
    ...
    private static class ByteArrayJavaFileObject extends SimpleJavaFileObject {
        ...
        public ByteArrayJavaFileObject(String name) {
            ...
            // ('file:' or 'jar:'); anything else forces the 'openOutputStream' callback for
            // Kind.CLASS
            super(
