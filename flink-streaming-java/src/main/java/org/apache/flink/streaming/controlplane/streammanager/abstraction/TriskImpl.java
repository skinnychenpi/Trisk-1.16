...
public final class TriskImpl implements ExecutionPlan {
    ...
    public ClassLoader getFunctionClassLoader() {
        ...
        for (OperatorDescriptor descriptor : jobConfigurations.values()) {
            ...
            // just need to find the first onc, since all function class should be loaded in one
            // classloader
            break;
        }
        if (funcObject == null) {
    ...
    public ExecutionPlan assignWorkload(
        ...
        for (OperatorDescriptor parent : targetDescriptor.getParents()) {
            // 做了两件事情：对要修改的operator的上游节点的key mapping进行更新（用最新的并行度下的key group进行覆盖），同时对自己这个operator的key
    ...
    public ExecutionPlan assignExecutionLogic(Integer operatorID, Object function) {
        Preconditions.checkNotNull(
                getUserFunction(operatorID), "previous key state allocation should not be null");
        OperatorDescriptor targetDescriptor = getOperatorByID(operatorID);
        try {
    ...
    public ExecutionPlan assignResources(
        ...
        // because we used cancel and redeploy method, the new deployed task should have a new task
        // id
        // the actual placement is to migrate entire state from one task to another, and then kill
        // the former.
        ...
        if (deployment != null) {
            if (deployment.size() != keyDistribution.size())
                throw new RuntimeException(
                        "++++++ inconsistent number of tasks in workload allocation and resource allocation.");
