...
public abstract class OperatorChain<OUT, OP extends StreamOperator<OUT>>
    ...
    public <T> RecordWriterOutput[] substituteRecordWriter(
        ...
        final RecordWriterOutput[] oldStreamOutputCopies =
                Arrays.copyOf(this.streamOutputs, this.streamOutputs.length);
        ...
        Map<IntermediateDataSetID, RecordWriterOutput<?>> streamOutputMap =
                new HashMap<>(outputsInOrder.size());
        ...
        Map<Integer, WatermarkGaugeExposingOutput<StreamRecord<T>>>
                watermarkGaugeExposingOutputMap = new HashMap<>();
        for (StreamConfig operatorConfig : chainedConfigs.values()) {
            @SuppressWarnings("unchecked")
            StreamOperator<T> operator =
                    (StreamOperator<T>) operatorMap.get(operatorConfig.getOperatorID());

            for (NonChainedOutput streamOutput :
                    operatorConfig.getOperatorNonChainedOutputs(userCodeClassloader)) {
                @SuppressWarnings("unchecked")
                RecordWriterOutput<T> output =
                        (RecordWriterOutput<T>) streamOutputMap.get(streamOutput.getDataSetId());
                operator.updateOutput(containingTask, output);
                // TODO scaling : what if multiple output, this is a problem!!!! @hya
            }
            //			WatermarkGaugeExposingOutput<StreamRecord<T>> output =
            //					generateOutputBaseExistOperator(containingTask, operatorConfig, streamOutputMap,
            // watermarkGaugeExposingOutputMap);
