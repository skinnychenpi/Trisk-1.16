...
public final class TriskImpl implements ExecutionPlan {
    ...
    @Internal
    public TriskImpl(
            Map<Integer, OperatorDescriptor> jobConfigurations,
    ...
    //            // just need to find the first onc, since all function class should be loaded in
    // one
    //            // classloader
    //            break;
    //        }
    ...
    //                getKeyStateAllocation(operatorID),
    //                "previous key state allocation should not be null");
    //        OperatorDescriptor targetDescriptor = getOperatorByID(operatorID);
    //        // update the key set
    //        //		targetDescriptor.updateKeyStateAllocation(distribution);
    //        Map<Integer, List<Integer>> upstreamTasks =
    //                transformations.getOrDefault("upstream", new HashMap<>());
    //        for (OperatorDescriptor parent : targetDescriptor.getParents()) {
    //            // 做了两件事情：对要修改的operator的上游节点的key mapping进行更新（用最新的并行度下的key
    // group进行覆盖），同时对自己这个operator的key
    //            // state distribution 覆盖为最新并行度下的key group
    ...
    //            Map<Integer, List<Integer>> downStreamTasks =
    //                    transformations.getOrDefault("downstream", new HashMap<>());
    ...
    //        Map<Integer, List<Integer>> updateStateTasks =
    //                transformations.getOrDefault("redistribute", new HashMap<>());
    ...
    //        Preconditions.checkNotNull(
    //                getUserFunction(operatorID), "previous key state allocation should not be
    // null");
    //        OperatorDescriptor targetDescriptor = getOperatorByID(operatorID);
    //        try {
    ...
    //        }
    //        // find out affected tasks, add them to transformations
    //        Map<Integer, List<Integer>> updateFunctionTasks =
    //                transformations.getOrDefault("updateExecutionLogic", new HashMap<>());
    //        updateFunctionTasks.putIfAbsent(
    //                targetDescriptor.getOperatorID(), targetDescriptor.getTaskIds());
    ...
    //        // because we used cancel and redeploy method, the new deployed task should have a new
    // task
    //        // id
    //        // the actual placement is to migrate entire state from one task to another, and then
    // kill
    //        // the former.
    //        Map<Integer, List<Integer>> keyDistribution =
    // targetDescriptor.getKeyStateDistribution();
    //        Map<Integer, List<Integer>> newKeyDistribution = new HashMap<>();
    //        if (deployment != null) {
    //            if (deployment.size() != keyDistribution.size())
    //                throw new RuntimeException(
    //                        "++++++ inconsistent number of tasks in workload allocation and
    // resource allocation.");
    //
    ...
    //            transformations.put("downstream", downStreamTasks);
    //
    //            Map<Integer, List<Integer>> reDeployingTasks =
    //                    transformations.getOrDefault("redeploying", new HashMap<>());
    //            reDeployingTasks.putIfAbsent(
    //                    targetDescriptor.getOperatorID(), targetDescriptor.getTaskIds());
    ...
    //            OperatorDescriptor operatorDescriptorCopy =
    //                    operatorDescriptor.copy(resourceDistributionCopy);
    ...
    //                OperatorDescriptor upstreamCopy =
    //                        jobConfigurationsCopy.get(upstream.getOperatorID());
    //                operatorDescriptorCopy.addParent(upstreamCopy);
    //            }
    //            for (OperatorDescriptor downstream : operatorDescriptor.getChildren()) {
    //                OperatorDescriptor downstreamCopy =
    //                        jobConfigurationsCopy.get(downstream.getOperatorID());
