...
/** @author trx The runner for the stream manager. */
public class StreamManagerRunnerImpl implements LeaderContender, StreamManagerRunner {
    ...
    /**
     * Lock to ensure that this runner can deal with leader election event and notifies
     * simultaneously.
     */
    ...
    public StreamManagerRunnerImpl(
        ...
        try {
            ...
            throw new JobExecutionException(
                    jobGraph.getJobID(), "Could not set up StreamManager", t);
        }
    }
        ...
        synchronized (lock) {
            if (!shutdown) {
                ...
                final CompletableFuture<Void> jobManagerTerminationFuture =
                        streamManagerService.closeAsync();
            ...
            leadershipOperation =
                    leadershipOperation.thenCompose(
                            (ignored) -> {
                                synchronized (lock) {
                                    //						return
                                    // verifySomethingAndStartStreamManager(leaderSessionID);
    ...
    private CompletionStage<Void> startStreamManager(UUID leaderSessionId) {
        ...
        try {
            ...
            return FutureUtils.completedExceptionally(
                    new FlinkException("Failed to start the StreamManager.", e));
        }

        final CompletableFuture<StreamManagerGateway> currentLeaderGatewayFuture =
                leaderGatewayFuture;
