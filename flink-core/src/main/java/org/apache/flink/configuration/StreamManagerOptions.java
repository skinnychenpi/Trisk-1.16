...
public class StreamManagerOptions {
     ...
     */
    @Documentation.Section(Documentation.Sections.ALL_JOB_MANAGER)
    public static final ConfigOption<String> STREAM_MANAGER_RESOURCE_ID =
            key("jobmanager.resource-id")
                    .stringType()
                    .noDefaultValue()
                    .withDescription(
                            "The StreamManager's ResourceID. If not configured, the ResourceID will be generated randomly.");

    /** The local address of the network interface that the stream manager binds to. */
    public static final ConfigOption<String> BIND_HOST =
            key("jobmanager.bind-host")
                    .stringType()
                    .noDefaultValue()
                    .withDescription(
                            "The local address of the network interface that the stream manager binds to. If not"
                                    + " configured, '0.0.0.0' will be used.");

    /** The local port that the stream manager binds to. */
    public static final ConfigOption<Integer> RPC_BIND_PORT =
            key("jobmanager.rpc.bind-port")
                    .intType()
                    .noDefaultValue()
                    .withDescription(
                            "The local RPC port that the StreamManager binds to. If not configured, the external port"
                                    + " (configured by '"
                                    + PORT.key()
                                    + "') will be used.");
