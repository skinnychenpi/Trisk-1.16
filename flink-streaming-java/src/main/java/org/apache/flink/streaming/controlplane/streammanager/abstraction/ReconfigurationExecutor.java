 ...
 * defined model to update the stream job execution plan which is maintained by stream manager.
 */
public interface ReconfigurationExecutor {
     ...
     *
     * @return the stream job state maintained in some place (e.g. {@link
     *     org.apache.flink.streaming.controlplane.streammanager.StreamManager})
     */
    ...
    /**
     * Use to notify job master that some operator inside job vertex changed, Thus the corresponding
     * executor could substitute new operator from the original one.
     *
     * <p>todo stream operator factory belongs to flink, should we decouple it?
     *
