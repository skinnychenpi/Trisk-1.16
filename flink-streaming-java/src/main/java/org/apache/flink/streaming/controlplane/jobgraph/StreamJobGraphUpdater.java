...
public class StreamJobGraphUpdater implements JobGraphRescaler, JobGraphUpdater {
    ...
    @Override
    public <OUT> JobVertexID updateOperator(
            int vertexId, OperatorDescriptor.ExecutionLogic executionLogic) throws Exception {
        ...
        if (factory instanceof SimpleOperatorFactory) {
            ...
            for (Map.Entry<String, Object> entry :
                    executionLogic.getControlAttributeMap().entrySet()) {
    ...
    public <OUT> JobVertexID updateOperator(
            OperatorID operatorID, StreamOperatorFactory<OUT> operatorFactory) throws Exception {
        return updateOperatorConfiguration(
                operatorID, config -> config.setStreamOperatorFactory(operatorFactory));
    }
     ...
     */
    private JobVertexID updateOperatorConfiguration(
            OperatorID operatorID, Consumer<StreamConfig> callBack) throws Exception {
        for (JobVertex vertex : jobGraph.getVertices()) {
            for (OperatorIDPair idPair : vertex.getOperatorIDs()) {
                OperatorID id = idPair.getGeneratedOperatorID();
                if (id.equals(operatorID)) {
                    if (vertex.getOperatorIDs().size() > 1) {
                        // there exists chained operators in this job vertex
                        updateConfigInChainedOperators(
                                vertex.getConfiguration(), operatorID, callBack);
                    } else {
    ...
    private void updateConfigInChainedOperators(
            Configuration configuration, OperatorID operatorID, Consumer<StreamConfig> callBack)
            throws Exception {
        StreamConfig streamConfig = new StreamConfig(configuration);
        Map<Integer, StreamConfig> configMap =
                streamConfig.getTransitiveChainedTaskConfigs(userClassLoader);
        for (StreamConfig config : configMap.values()) {
